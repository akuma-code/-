...
function addListener() { //добавляет на поля ввода размеров возможность считать по нажатию ентера
    ...
    for (let size of sizes) {
        size.addEventListener("keyup", function(event) {
            if (event.keyCode === 13 && (event.ctrlKey || event.altKey)) {
                ...
                mid.dataset.calcedvalue = document.getElementById('w').value - document.getElementById('levo').value - document.getElementById('pravo').value
            };

            if (event.keyCode === 13) {
                mid.dataset.calcedvalue = document.getElementById('w').value - document.getElementById('levo').value - document.getElementById('pravo').value;
                mid.value = document.getElementById('w').value - document.getElementById('levo').value - document.getElementById('pravo').value;
                event.preventDefault();
    ...
    door.addEventListener("click", function() {
        ...
        document.getElementById('himp').style.display = (door.dataset.isfix == "1") ? "block" : "none";
    });
