function addListener() { //добавляет на поля ввода размеров возможность считать по нажатию ентера
    ...
    for (let size of sizes) {
        ...
        size.addEventListener("mousedown", function(event) {
            // event.preventDefault();
            this.value = "";
        })
    };
    ...
    door.addEventListener("click", function() {
        document.getElementById('himp').style.display = (door.dataset.isfix == "0" && document.getElementById('imgbox').dataset.isbb === true) ? "block" : "none";
    });
