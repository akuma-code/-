function gonow() {
    ...
    if (store.has('summ')) {
        ...
        store.set('summ', temp);
        // console.log(` Всего: ${Math.ceil(store.get("summ") * .6)} руб.`);
...
Summary: ${sum} rub`);
    out.toDiv(zh);
    document.getElementById('calc-btn').innerHTML = `${Math.ceil(store.get("summ") * .6)} руб.`
    document.getElementById('outside').insertAdjacentHTML('beforeend', `<div class="summ">Скидка 40%: <b>${Math.round(sum*.6)} руб.</b></div> `)
        // document.getElementById('outside').insertAdjacentHTML('beforeend', `<div class="summ">Итого: <b>${Math.ceil(store.get("summ") * .6)} руб.</b></div> `)
