...
function calc_rs() {
    ...
    output.insertAdjacentHTML("beforeend", `<div class="rs_out">${calc.type} ::: ${$w.value}мм x ${$h.value}мм<br>
${$color}<b> ==>  ${Math.round(calc.calcIt($w.value, $h.value)*$disc)} руб.</b></div>`);
}
